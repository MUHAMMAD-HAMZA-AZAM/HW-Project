...
@Component({
  selector: 'supplier-product-list.component',
...
export class SupplierProductListComponent implements OnInit {
  ...
  ngOnInit() {
    this.userRole = JSON.parse(localStorage.getItem("Supplier Product List"));
  ...
  public showDetailsModal(prodId, productDetailsModel) {
    ...
    this.getSupplierProductImagesByProductId(this.productId);
    this._modalService.open(productDetailsModel, { size: 'lg', centered: true });
  }

  //----------------- Show Product Images by Product Id

  getSupplierProductImagesByProductId(productId) {
    this.Loader.show();
  ...
  handleChange(event, productId) {
    ...
    this.common.get(this.common.apiRoutes.Supplier.BlockProduct + "?productId=" + productId + "&status=" + status).subscribe(result => {
      let response = result.json();
      if (response.status == 200) {
        if (status) {
          this.toaster.success("Product Unblocked successfully!", "Success");
        }
        else {
          this.toaster.warning("Product blocked successfully!", "Warning");
        }
        this.getSupplierProducsListForApprovel();
      }
    })
