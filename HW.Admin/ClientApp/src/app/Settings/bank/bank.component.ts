...
export class BankComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.userRole = JSON.parse(localStorage.getItem("Bank"));
  ...
  public getBanksList() {
    debugger;
    this.Loader.show();
    this._httpService.get(this._httpService.apiRoutes.Supplier.GetBanksListForAdmin).subscribe(result => {
      this.response = result.json();
      if (this.response.status == StatusCode.OK) {
        this.banklist = this.response.resultData;
        this.Loader.hide();
      }
...
debugger;
  /*  this.decodedtoken.UserId */
    ...
    this._httpService.PostData(this._httpService.apiRoutes.Supplier.deletebankStatus, JSON.stringify(obj), true).then(res => {
  ...
  //-------------------  Show Modal For Add bank/........................
  public showModal(content) {
    this.modelName = "Add  new Bank";
    this._modalService.open(content, { backdrop: 'static', keyboard: false });
  }

  //-------------------  Show Modal For Update bank.......................//
  public update(data, content) {
debugger;
    this.Loader.show();
    this.btnReset = true;
    this.showModal(content);
    this.modelName = "Update Bank";
    this.appValForm.patchValue(data);
    this.Loader.hide();
  }
  /* ................. add update bank name.................*/
  public saveAndUpdateCountry() {
    this.formData = this.appValForm.value;
    this.formData.userId = this.decodedtoken.UserId
    this.Loader.show();
    this._httpService.PostData(this._httpService.apiRoutes.Supplier.AddUpdatebank, JSON.stringify(this.formData), true).then(res => {
      this.response = res.json();
      if (this.response.status == StatusCode.OK) {
        if (this.response.resultData) {
          this.toaster.error("Bank Name already Exist", "Alert", { timeOut: 5000 });
        }
        else {
          this.toaster.success(this.response.message, "Success");
        }
  ...
  public resetForm() {
    ...
    this.appValForm.controls.bankId.setValue(0);
  }
