...
@Component({
  selector: 'app-manual-reports-bids',
  templateUrl: './manual-reports-bids.component.html',
  styleUrls: ['./manual-reports-bids.component.css']
})
export class ManualReportsBidsComponent implements OnInit {
  ...
  ngOnInit() {
    this.userRole = JSON.parse(localStorage.getItem("Bids"));
  ...
  DownloadXlsx() {
    this.excelService.exportAsExcelFile(this.excelFileList, "BidsReport")
  }
  ...
  DownloadPdf() {
    ...
    //    pdf.save('BidsManualReport.pdf'); // Generated PDF  
    //    this.Loader.hide();
    //  });
  ...
  public loadJobs() {
    ...
    //  if (fromdata.tradesmanSearch == "4") {
    //    tradesmanpra = this.tradesmanName + "%";
    //  }
    //  else if (fromdata.tradesmanSearch == "5") {
    //    tradesmanpra = "%" + this.tradesmanName + "%";
    //  }
    //  else if (fromdata.tradesmanSearch == "6") {
    //    tradesmanpra = "%" + this.tradesmanName;
    //  }
    //}
    ...
    //}
    debugger
    query = this.service.apiRoutes.Jobs.getBisForDynamicReport + "?postedstartDate=" + this.startDate1 + "&postedendDate=" + this.endDate1 + "&endfromDate=" + this.fromDate1 + " &endtoDate=" + this.toDate1 + "&customer=" + customerpra + "&tradesman=" + tradesmanpra + "&status=" + sids + "&city=" + ctids + "&lastActive=" + this.lastActive + "&location=" + this.location + "&jobId=" + this.jobId + "&customerId=" + this.customerId;
    ...
    this.service.get(query).subscribe(result => {
      ...
      if (this.bidsList) {
        this.excelFileList = this.bidsList.map((bid) => {
          return {
            jobQuotationId: bid.jobQuotationId,
            bidsId: bid.bidsId,
            customerName: bid.customerName,
            comments: bid.comments,
            amount: bid.amount,
            status: bid.status,
            createdOn: new Date(bid.createdOn).toLocaleDateString(),
          }
        })
        this.reportFilter = true;
        this.showTable = true;
        this.showNullMessage = false;
        this.jobcount = this.bidsList.length;
      }
      }
      else {
