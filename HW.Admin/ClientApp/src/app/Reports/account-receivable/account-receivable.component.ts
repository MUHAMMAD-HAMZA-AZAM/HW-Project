...
export class AccountReceivableComponent implements OnInit {
  ...
  public getAccountReceiveableUserNames() {
    this.Loader.show();
    this.common.get(this.common.apiRoutes.PackagesAndPayments.GetAccountUsersName + "?accountType=" + this.accounts.Receiveables).subscribe(result => {
  ...
  public showDetailedLedgerTransectionScreen() {
    ...
    if (!this.appValForm.invalid) {
      ...
      formData.accountId = this.accounts.Receiveables;
      this.Loader.show();
