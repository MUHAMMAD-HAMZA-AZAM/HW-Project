...
@Component({
  selector: 'app-tradesman-report',
  templateUrl: './tradesman-report.component.html',
  styleUrls: ['./tradesman-report.component.css']
})
export class TradesmanReportComponent implements OnInit {
  ...
  populateTradesmanReport() {
    ...
    this.Loader.show();
    this.service.get(this.service.apiRoutes.Users.getAllTradesmanWithDatesReport + "?startDate=" + this.startDate + "&endDate=" + this.endDate).subscribe(result => {
  ...
  DownloadPdf() {
    ...
    this.dynamicScripts.load("pdf").then(data => {
      ...
      html2canvas(htmltable).then(canvas => {
        ...
        pdf.save('TradesmanCompleteReport.pdf'); // Generated PDF  
      }); 
    })
    this.Loader.hide();
  }
